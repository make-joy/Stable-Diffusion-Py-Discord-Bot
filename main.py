...
# 도움말 명령어
help_cmds = ['그림봇', 'how', 'rmflaqht', 'ㅙㅈ', 'ㅗㅐㅈ']

# 그리기 명령어
draw_cmds = ['그림', '그리기', 'rmfla', 'rmflrl', 'dr', 'ㅇㄱ', 'ㅇㄱㅁㅈ']

# 디스코드 봇 답장용 헤더
bot_header = "[MJ Draw Bot] "
...
@bot.event
async def on_ready():	
	await bot.change_presence(status = discord.Status.online, activity = discord.Game(f"{prefixs[0]}{help_cmds[0]} or {prefixs[0]}{help_cmds[1]}"))
...
def translate(text):
	...
	response = requests.post(tr_url, data = payload)
	res = response.json()
...
# 설치된 모든 모델 불러오기
model_data  = requests.get(f"{url}/sdapi/v1/sd-models").json()
models = {}
...
# 모든 샘플러 불러오기
sampler_data  = requests.get(f"{url}/sdapi/v1/samplers").json()
samplers = {}
...
# 현재 모델 불러오기
now_model = requests.get(f"{url}/sdapi/v1/options").json().get("sd_model_checkpoint", None)
...
async def draw(ctx, *, message):
	...
	try:
		...
		async with aiohttp.ClientSession() as session:
			...
			# 모델 변경
			option_response = await session.post(f"{url}/sdapi/v1/options", json = option_payload)

			# txt2img api 호출 및 생성
			async with session.post(f"{url}/sdapi/v1/txt2img", json = payload) as payload_response:
			...
			for i in r['images']:
				...
				async with session.post(f"{url}/sdapi/v1/png-info", json = png_payload) as png_response:
	...
	# 오류 예외처리
		...
		if "translations" in str(e):
			...
			await ctx.reply(f"```ini\n{bot_header}올바른 명령어 사용법을 확인해주세요.\n\n도움말 보기 : {prefixs[0]}{help_cmds[0]} or {prefixs[0]}{help_cmds[1]}\n```")
